...
PDFJS.imageResourcesPath = '/components/pdfjs/web/images/';
PDFJS.workerSrc = '/components/pdfjs/build/pdf.worker.js';
PDFJS.cMapUrl = '/components/pdfjs/web/cmaps/';
...
var DEFAULT_SCALE = 'page-width'; // auto
var UNKNOWN_SCALE = 0;
...
function scrollIntoView(element, spot) {
  ...
  if (!parent) {
    console.log('offsetParent is not set -- cannot scroll');
    return;
  }
...
(function mozPrintCallbackPolyfillClosure() {
  ...
  window.addEventListener('keydown', function(event) {
    if (!myPresenterFlag || !isDocumentScrollable) return;
...
var SecondaryToolbar = {
  ...
  openFileClick: function secondaryToolbarOpenFileClick(evt) {
    ...
    document.getElementById("pdf-file-open").click();
    this.close();
  },
  ...
  open: function pdfViewOpen(file, scale, password,
    ...
    PDFJS.getDocument(parameters, pdfDataRangeTransport, passwordNeeded,
      ...
      function getDocumentCallback(pdfDocument) {
        ...
        self.loading = false;
        if (!myPresenterFlag) request_owner_doc_state();
      },
...
window.addEventListener('change', function webViewerChange(evt) {
  ...
  //
  //var file = files[0];
  //if (myPresenter_flag && file.size > 15728640) {
  //  alert('File size can not be over 15MByte.');
  //  return;
  //}
  var file = files[0];
  if (file.size > 5242880){
    alert('File size can not be over 5MByte.');
    show_loading(false);
    return;
  }
  ...
  if (fileType == "application/pdf"
    ...
    reader.addEventListener('loadend', function () {
      ...
      /*var xhr = new XMLHttpRequest();
      xhr.open('POST', '/utils/file2pdf', true);
      xhr.setRequestHeader("Content-Type", "application/octet-stream");
      xhr.setRequestHeader("x-course-id", courseData.courseID);
      ...
      xhr.onreadystatechange = function() {
        if(xhr.readyState == 4 && xhr.status == 200) {
          var req_ans = JSON.parse(xhr.responseText);
          if (req_ans.err == 0){
            insert_new_pdf_item(req_ans.item);
            // disable auto draw document
            if (false){
              send_open_file_msg(req_ans.item.downurl);
              if (fileType == "application/pdf") {
                ...
                PDFViewerApplication.open(req_ans.item.downurl, 0);
              }
            }
          } else {
            alert(req_ans.msg);
...
window.addEventListener('keydown', pdf_keydown);

function pdf_keydown(evt) {
  if (!myPresenterFlag || !isDocumentScrollable) {
    //evt.preventDefault();
    return;
  }
